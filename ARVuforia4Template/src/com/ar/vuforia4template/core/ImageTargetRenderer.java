...
import com.ar.vuforia4template.utils.LoadingDialogHandler;
import com.ar.vuforia4template.utils.SampleApplicationSession;
import com.ar.vuforia4template.utils.SampleMath;
import com.ar.vuforia4template.utils.SampleUtils;
import com.ar.vuforia4template.utils.Texture;
...
public class ImageTargetRenderer implements GLSurfaceView.Renderer {
	...
	private ActivityImageTargets _activity;
	...
	private Map<String, Vec3F> _targetPositiveDimensions = new TreeMap<String, Vec3F>();

	public ImageTargetRenderer(ActivityImageTargets activity,
...
//			}
	...
	boolean isTapOnScreenInsideTarget(String targetName, float x, float y) {
		...
		Vec3F vec3f = _targetPositiveDimensions.get(targetName);
		...
		if ((intersection.getData()[0] >= -(vec3f.getData()[0]))
				&& (intersection.getData()[0] <= (vec3f.getData()[0]))
				&& (intersection.getData()[1] >= -(vec3f.getData()[1]))
				&& (intersection.getData()[1] <= (vec3f.getData()[1])))
			return true;
		else
			return false;
